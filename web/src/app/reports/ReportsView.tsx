...
function buildPeriodQuery(period: ReportPeriod) {
  ...
  const params = new URLSearchParams({
    date_from: toLocalDateTime(start),
    date_to: toLocalDateTime(end, true),
  });
...
export default function ReportsPageContent() {
  ...
  const units = filteredSales.reduce((sum, sale) => sum + sale.items.reduce((itemSum, item) => itemSum + item.quantity, 0), 0);
                ...
                {filteredSales.map((sale) => (
                  <article className={styles.saleCard} key={sale.id}>
                    ...
                    <div className={styles.items}>
                      {sale.items.map((item) => (
                        <div key={`${sale.id}-${item.product_id}`}>
                          <span>
                            {productDetails.get(item.product_id)?.name ?? `Produit #${item.product_id}`}
                            {productDetails.get(item.product_id)?.image_url ? (
                              <img src={productDetails.get(item.product_id)?.image_url ?? ""} alt="" className={styles.productThumb} />
                            ) : null}
                          </span>
                          <small>
