...
export default function UsersPage() {
  ...
  async function loadUsers() { const response = await fetch(`${API_URL}/api/v1/auth/users`, { credentials: "include" }); if (response.status === 403) throw new Error("Cette page est réservée aux administrateurs."); if (!response.ok) throw new Error("Impossible de charger les utilisateurs."); setUsers(await response.json()); }
