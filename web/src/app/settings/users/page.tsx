...
export default function UsersPage() {
  ...
  async function loadUsers() { const response = await fetch(`${API_URL}/api/v1/auth/users`, { headers: authHeaders(), credentials: "include" }); if (response.status === 403) throw new Error("Cette page est réservée aux administrateurs."); if (!response.ok) throw new Error("Impossible de charger les utilisateurs."); setUsers(await response.json()); }
  useEffect(() => { loadUsers().catch((reason: Error) => setError(reason.message)).finally(() => setIsLoading(false)); }, []);
  async function createUser(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const response = await fetch(`${API_URL}/api/v1/auth/users`, { method: "POST", headers: { "Content-Type": "application/json", ...authHeaders() }, credentials: "include", body: JSON.stringify(form) }); if (!response.ok) { setError("Impossible de créer ce compte. Vérifiez l’email et le mot de passe."); return; } setForm(emptyForm); setShowForm(false); setError(""); await loadUsers(); }
