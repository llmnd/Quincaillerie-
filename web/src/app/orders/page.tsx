...
export default function OrdersPage() {
  ...
  useEffect(() => { fetch(`${API_URL}/api/v1/sales`, { headers: authHeaders(), credentials: "include" }).then(async (response) => { if (!response.ok) throw new Error(); setSales(await response.json()); setMessage(""); }).catch(() => setMessage("Les ventes ne sont pas disponibles pour le moment.")); }, []);
